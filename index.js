...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            const url = `https://polygon-scrimba-test.monik182.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`
...
async function fetchReport(data) {
    ...
    try {
        const url = 'https://ai-api-worker.monik182.workers.dev/'
